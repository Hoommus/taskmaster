...
enum e_input_state	toggle_quotes(enum e_input_state current, u_int64_t i)
{
	...
	if (current < STATE_QUOTE && buff_char_at_equals(i, "'"))
		...
		return (g_shell->fallback_input_state);
...
enum e_input_state	recheck_state(u_int64_t from_index)
{
	...
	if (g_shell->input_state == STATE_HEREDOC ||
		...
		return (g_shell->input_state);
	current = g_shell->fallback_input_state;
	i = from_index;
	while (i < g_shell->buffer->size)
	{
	...
	return (g_shell->input_state = current);
}
