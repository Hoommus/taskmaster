...
t_carpos	*carpos_load(enum e_position type)
{
	if (g_shell->input_state != STATE_NON_INTERACTIVE)
		tputs(tgoto(tgetstr("cm", NULL), g_shell->carpos_db[type].col,
				g_shell->carpos_db[type].row), 1, &ft_putc);
	return (g_shell->carpos_db + type);
}
...
t_carpos	*carpos_update(enum e_position type)
{
	...
	if (g_shell->tty_fd != -1 && g_shell->input_state != STATE_NON_INTERACTIVE)
	{
		write(2, "\x1b[6n", 4);
		response[16] = 0;
		...
		{
			g_shell->carpos_db[type].row =
				(short)(ft_atoi(ft_strchr(response, '[') + 1) - 1);
			g_shell->carpos_db[type].col =
				(short)(ft_atoi(ft_strchr(response, ';') + 1) - 1);
		}
	}
	return (g_shell->carpos_db + type);
