...
void						tpool_init(void)
{
	...
	log_write(TLOG_DEBUG, "Thread pool initialized");
}
												...
												void *arg)
{
	...
	i = -1;
	status = -1;
	log_write(TLOG_DEBUG, "Locking a tpool mutex");
	pthread_mutex_lock(&g_mutex_thread_pool);
	while (++i < g_thread_pool->pool_capacity)
		if (!g_thread_pool->is_busy[i])
		{
			log_write(TLOG_DEBUG, "Found not busy thread, assigning id");
			((struct s_thread_args *)arg)->thread_id = i;
			if ((status =
				pthread_create(g_thread_pool->thread + i, attr, runnable, arg)))
				break ;
			log_write(TLOG_DEBUG, "pthread created successfully, assigning args");
			g_thread_pool->args[i] = arg;
			log_write(TLOG_DEBUG, "marking thread busy");
			g_thread_pool->is_busy[i] = true;
			log_write(TLOG_DEBUG, "incrementing number of running threads");
			g_thread_pool->threads_number++;
			status = 0;
			break ;
		}
	log_write(TLOG_DEBUG, "unlocking a tpool mutex");
...
{
	log_fwrite(TLOG_DEBUG, "Finalising %d thread", id);
	pthread_mutex_lock(&g_mutex_thread_pool);
	g_thread_pool->threads_number--;
